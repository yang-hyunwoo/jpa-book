...
public class JWTSimpleTest {
    ...
    void test_1() {
        String okta_token = Jwts.builder().addClaims(
                        Map.of("name", "hyunwoo", "price", 3000)
                ).signWith(SignatureAlgorithm.HS256, "hyunwoo")
                .compact();
        System.out.println(okta_token);
        printToken(okta_token);

        Jws<Claims> tokenInfo = Jwts.parser().setSigningKey("hyunwoo").parseClaimsJws(okta_token);
        System.out.println(tokenInfo);

    }
    /*
    jwt 와 jjwt 의 차이점은 키의 해싱 방법이다.
    jjwt 같은 경우 알고리즘 키의 값을 해싱을 한다.
    java-jwt 같은 경우는 키의 값을 해싱을 하지 않는다.
    그래서 키의 값이 다르게 나온다.
    같이 사용을 하기 위해서는 DatatypeConverter.parseBase64Binary() 값을 키를 같이 검증을 하면 된다.
     */
    ...
    void test_2() {
        ...
        byte[] SEC_KEY = DatatypeConverter.parseBase64Binary("hyunwoo");
        String oauth0_token = JWT.create().withClaim("name", "hyunwoo").withClaim("price", 3000)
    ...
    void test_3() throws InterruptedException {
        final Algorithm AL = Algorithm.HMAC256("hyunwoo");
        String token = JWT.create().withSubject("hyunwoo")
