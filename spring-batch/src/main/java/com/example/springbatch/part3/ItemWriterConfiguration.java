...
//                .next(this.jdbcBatchItemWriterStep())
    ...
    private ItemWriter<Person> jpaItemWriter() throws Exception {
        JpaItemWriter itemWriter = new JpaItemWriterBuilder<Person>()
                .entityManagerFactory(entityManagerFactory)
//                .usePersist(true)       //insert시 update할지말지 체크
                .build();
    ...
    private ItemWriter<Person> csvFileItemWriter() throws Exception {
        ...
        FlatFileItemWriter<Person> itemWriter =new FlatFileItemWriterBuilder<Person>()
                ...
                .encoding("UTF-8")
                .resource(new FileSystemResource("output/test-output.csv"))        //파일write //파일 read classPathResource
                .lineAggregator(lineAggregator)
                .headerCallback(writer -> writer.write("id,이름,나이,거주지"))
                .footerCallback(writer -> writer.write("-------------------\n"))
                .append(true)       //덮에쓸지말지 여부  true(덮어쓰기 x)
                .build();
        itemWriter.afterPropertiesSet();
        return itemWriter;
    }
